  ...
  const generar = async () => {
    ...
    try {
      ...
      for (let i = 24; i < novaData.length; i++) {
        ...
        gruposConIndices.forEach(grupo => {
          grupo.indices.forEach(idx => {
            ...
            if (grupo.columnaDestino === 'Health Cost' && val <= 1000) return
      ...
      for (const cliente of clientesSeleccionados) {
        ...
        const TOTAL_OTHER_EXTRA  = ['LEAVE PAID REFOUND', 'PARKING COST', 'SENA QUOTE', 'LIEU OF NOTICE']
        ...
        const todasNumCols = new Set([
          ...
          ...['LEAVE PAID REFOUND', 'PARKING COST', 'SENA QUOTE'].map(h => postCols[h]).filter(c => c && c !== -1),
